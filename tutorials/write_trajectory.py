...
def write_commonroad_trajectory(json_data):
    ...
    for i in range(len(x)):
        ...
        state = State(position=np.array([x[i], y[i]]), velocity=v_long[i], orientation=orient[i],
                      steering_angle=steering[i], time_step=i, hitch=hitch[i])
        states.append(state)
    return states
...
if __name__ == "__main__":
    ...
    output_npy_file = 'trajectory'
    np.save(output_npy_file, data)
    print("Trajectory saved to {}.npy".format(output_npz_file))
