...
\begin{center} {\Huge\textbf{CommonRoad: Documentation of the XML Format} \\[0.3cm] \huge(Version 3.0)} \end{center}
\vspace{0.2cm}
...
Within \textit{CommonRoad}\cite{Althoff2017a}\footnote{\href{https://commonroad.in.tum.de}{commonroad.in.tum.de}}, XML files are used to store the data for specific driving scenarios. 
In this documentation, we present the definition of CommonRoad scenarios and XML files, which are composed of (1) meta information describing the scenario (see Section~\ref{subsec:meta}), (2) a formal representation of the road network (see Section~\ref{subsec:lanelets}-\ref{subsec:intersections}), (3) static and dynamic obstacles (see Section~\ref{subsec:obstacles}), and (4) the planning problem of the ego vehicle(s) (see Section~\ref{subsec:egoVehicles}). 
...
Throughout this documentation, we provide some snippets to illustrate the implementation in XML. 
For more examples, see the example file \texttt{XML\_commonRoad\_minimalExample.xml} or any benchmark scenario on our website. 
...
Since CommonRoad version 3.0, a CommonRoad scenario is represented by two XML files instead of a single file.
We have separated the static and dynamic information. 
As a brief introduction, the following snippet shows the two \textit{CommonRoad} XML files. Each file consists of an XML header (XML version and encoding type) and the \textit{CommonRoad} root element. The first file represents the static data which consists of location, lanelets, traffic signs, traffic lights, intersection elements:
\begin{lstlisting}
<?xml version="1.0" encoding="utf-8"?>
<commonRoad commonRoadVersion='3.0' benchmarkID='short_benchmark_id' 
...
\end{lstlisting}
The second file represents the dynamic data which includes, static and dynamic obstacles, and planning problems:
\begin{lstlisting}
	<?xml version="1.0" encoding="utf-8"?>
	<commonRoad commonRoadVersion='3.0' benchmarkID='long_benchmark_id' 
	date='2022-02-01' author="Sebastian Maierhofer" affiliation="Technical University of 
	Munich, Germany" source="Next Generation Simulation (NGSIM) and OpenStreetMaps 
	(OSM)" timeStepSize='0.1'>
	<scenarioTags>
...
\item New scenario version definition
\end{itemize}
...
The overall structure of the XML definition is shown by Fig.~\ref{fig:XMLstructure}. 
...
The \textit{CommonRoad} root element has the following attributes (its elements are shown in Fig.~\ref{fig:XMLstructureStatic} and Fig.~\ref{fig:XMLstructureDynamic}):
\begin{itemize}
...
\paragraph{Version} = $\{0$-$9\}^*$-$\{0$-$9\}^*$-$\{0$-$9\}^*$ specifies the scenario version. The first two numbers represent the CommonRoad version. The last number is optional and represents minor updates in the scenario itself, e.g., if a small bug is fixed. Note that before CommonRoad version 3.0, we had a different the naming convention of the scenario version. We changed the CommonRoad version ID from year-based to number-based. Tab. \ref{tab:scenario_version_ids} maps the year-based version representation to a number-based for versions. 
