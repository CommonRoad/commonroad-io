...
\begin{center} {\Huge\textbf{CommonRoad: Documentation of the XML Format} \\[0.3cm] \huge(Version 3.0.0)} \end{center}
\vspace{0.2cm}
\begin{center} {\large Sebastian Maierhofer, Markus Koschi, Anna-Katharina Rettinger, Stefanie Manzinger, and Matthias Althoff} \\[0.2cm] Technische Universit\"at M\"unchen, 85748 Garching, Germany
\end{center}
\vspace{0.2cm}
...
\begin{abstract}
This document presents the XML format in \textit{CommonRoad} for specifying road traffic scenarios. 
%The provided scenarios in \textit{CommonRoad} are described in the scenario documentation.
The \textit{CommonRoad} XML files are composed of (1) meta information about the scenario, (2) a formal representation of the road network, (3) static and dynamic obstacles, and (4) the planning problem for the ego vehicle(s). %In order to unambiguously represent traffic scenarios, we have developed the \textit{CommonRoad} XML format. %based on the OpenStreetMap (OSM) format so that we can use road data from OSM.
%In order to use road data from OpenStreetMap (OSM) for the road network and visualize it using standard software, our XML format is based on the OSM format. 
...
Within \textit{CommonRoad}\cite{Althoff2017a}\footnote{\href{https://commonroad.in.tum.de}{commonroad.in.tum.de}}, XML files are used to store the data for specific driving scenarios. In this documentation, we present the definition of the XML files, which are composed of (1) meta information describing the scenario (see Section~\ref{subsec:meta}), (2) a formal representation of the road network (see Section~\ref{subsec:lanelets}-\ref{subsec:intersections}), (3) static and dynamic obstacles (see Section~\ref{subsec:obstacles}), and (4) the planning problem of the ego vehicle(s) (see Section~\ref{subsec:egoVehicles}). In non-collaborative scenarios, only one planning problem exists, while in collaborating scenarios several planning problems have to be solved.

Throughout this documentation, we provide some snippets to illustrate the implementation in XML. For more examples, see the example file \texttt{XML\_commonRoad\_minimalExample} or any benchmark scenario on our website. A visualization of all scenarios is on our website\footnote{\href{https://commonroad.in.tum.de/scenarios/}{commonroad.in.tum.de/scenarios}}, where you can also search for specific types of scenarios.

After CommonRoad version 2020a, we have changed the naming convention of the scenario version from year-based to number-based. Tab. ABC maps the year-based version representation to a number-based for versions until 2020a. 
\begin{table}[!htb]\centering
	\caption{Year-base with corresponding number-based version.}
	\ra{1.3} 
	\begin{tabular}{@{}lll@{}} \toprule
		\textbf{year-based} & \textbf{number-based} & \textbf{scenario ID}  \\
		2017a & 0.1 & 01\\
		2017b & 0.2 & 02\\
		2018a & 0.3 & 03\\
		2018b & 1.0 & 10\\
...
Since CommonRoad version 3.0, a CommonRoad scenario is represented by two XML files instead of one single file. However, the content itself didn't change. We have only separated the static and dynamic information into two files. 
As a brief introduction, the following snippet shows that a \textit{CommonRoad} XML file consists of an XML header (XML version and encoding type) and the \textit{CommonRoad} root element with location, scenario tag, lanelet, traffic sign, traffic light, intersection element, static and dynamic obstacle, and planning problem:
\begin{lstlisting}
<?xml version="1.0" encoding="utf-8"?>
<commonRoad commonRoadVersion='3.0.0' benchmarkID='documentation_example' 
date='2022-02-01' author="Sebastian Maierhofer" affiliation="Technical University of 
Munich, Germany" source="Next Generation Simulation (NGSIM) and OpenStreetMaps 
(OSM)" timeStepSize='0.1'>
    <location>
    	...
    </location>
    <scenarioTags>
    ...
    ...
    <staticObstacle id='50'>
        ...
    </staticObstacle>
    ...
    <dynamicObstacle id='60'>
    	...
    </dynamicObstacle>
    ...
...
For a quick reference, we summarize the major changes of version 3.0 compared to version 2.0 (2020a):
\begin{itemize}
\item New intersection definition
\item Separation of static and dynamic information into two files
\item New version definition
\end{itemize}

\section{Specification of the XML Format}
...
All variables are given by decimal numbers based on SI units. We use a common Cartesian coordinate frame with x-, y-, and z-axis to be able to represent all road networks including bridges and tunnels. If a scenario is only defined in a two-dimensional plane (which is often the case), we use the convention that all z-coordinates are zero.
Angles are measured counter-clockwise around the positive z-axis with the zero angle along the x-axis.


The overall structure of the XML definition is shown by Fig.~\ref{fig:XMLstructure}. The \textit{CommonRoad} root element contains one or more elements of type \textit{lanelet}, \textit{staticObstacle}, \textit{dynamicObstacle}, \textit{trafficSign}, \textit{trafficLight}, \textit{intersection}, and \textit{planningProblem}. Each has a unique\footnote{Unique within the whole XML document.} ID (of type positive integer) making it possible to reference it. Additionally, the root element contains a \textit{location} and \textit{tag} to describe a scenario.
...
\begin{figure}[!htpb]
	...
	\dirtree{%
		...
		% .4 [2] point.
		% .4 [0..1] lineMarking.
		% .4 [0..1] trafficSignRef (\textrm{ref to} trafficSign).
		% .4 [0..1] trafficLightRef (\textrm{ref to} trafficLight).
		%.3 [0..1] role: vehicle/bike/pedestrian.
...
\begin{figure}[!htpb]
	...
	\caption{Structure of the XML files encoding each scenario}
	\label{fig:XMLstructure}
\end{figure}
\newpage

%The specification of each element type is presented in this section. In Sec.~\ref{subsec:auxiliary}, we will introduce all auxiliary elements (see Fig.~\ref{fig:XMLauxiliary}). Then, we will use the auxiliary elements to build the elements \textit{lanelet} (Sec.~\ref{subsec:lanelets}), \textit{obstacle} (Sec.~\ref{subsec:obstacles}), and \textit{planningProblem} (Sec.~\ref{subsec:egoVehicles}).
...
The \textit{CommonRoad} root element has the following attributes (its elements are shown in Fig.~\ref{fig:XMLstructure}):
\begin{itemize}
...
%\item \texttt{tags}: keywords describing the type of scenario (see Sec.~\ref{subsec:tags}),
...
\paragraph{Examples:} Possible examples of a benchmark ID are: C-USA\_US101-1\_123\_T-1, DEU\_FFB-2\_42\_S-4, DEU\_Hhr-1\_1.
...
%\subsubsection{GeoReference Element}
%\label{sec:georeference-element}
...
%\subsubsection{AdditionalTransformation Element}
%\label{sec:addit-elem}
...
Additionally, the filtering based on the number of static obstacles, dynamic obstacles, obstacle types, type of future behavior of obstacles, number of ego vehicles, number of goal states, and time horizon of the scenario is possible. %, and several location elements is possible.
...
\begin{table}[!htb]\centering
	...
	\begin{tabular}{@{}ll@{}} \toprule
		\textbf{LaneletType} & urban, country, highway, driveWay, mainCarriageWay, accessRamp, exitRamp, \\
		 & shoulder, busLane, busStop, bicycleLane, sidewalk, crosswalk, interstate, intersection unknown\\
...
The element \textit{intersection} is used to represent an intersection within the road network (see Fig.~\ref{fig:XML_intersection}). An \textit{intersection} element is defined by at least one \textit{incoming}, which consist of \textit{incomingLanelets}, \textit{successorsRight}, \textit{successorsStraight}, \textit{successorsLeft}, the element \textit{isLeftOf}, and the element \textit{crossing}. 

The elements \textit{successorsRight}/\textit{Straight}/\textit{Left} are used to infer for which lanelets the traffic light is valid for and to facilitate the calculation of priorities at intersections. The element \textit{isLeftOf} is used to infer the right-before-left-rule. The element \textit{crossing} models lanelets which cross other lanelets, e.g., crosswalks. Since all elements of a \textit{incoming} are already present in the XML file, we use an attribute referring to their unique ID. An example for an \textit{intersection} with all elements except crossings is illustrated in Fig.~\ref{fig:intersection}.
\begin{figure}[!htb]
	\centering
	\footnotesize
	\psfrag{A}[l][c]{incoming}	
	\psfrag{B}[l][c]{successorsRight}
	\psfrag{C}[l][c]{successorsStraight}
	\psfrag{D}[l][c]{successorsLeft}
	\psfrag{E}[l][c]{isLeftOf}
	\psfrag{F}[c][c]{incomingLanelet}
	\includegraphics[width=0.6\columnwidth]{figures/intersection.eps}
	\caption{Example intersection. For better visibility, only one example lanelet is highlighted for each successorsRight/Straight/Left element.}
...
\begin{figure}[!htb]
	...
	%	.3 [0..N] trafficSign.
	%	.3 [0..N] trafficLight.
		.3 [0..N] successorsRight (\textrm{ref to} all lanelets that are turning right within this intersection (and are successors of the incomingLanelet(s))).
		.3 [0..N] successorsStraight (\textrm{ref to} all lanelets that are going straight).
		.3 [0..N] successorsLeft (\textrm{ref to} all lanelets that are turning left).
...
\bibliography{althoff_own,althoff_other,Althoff2017a}
