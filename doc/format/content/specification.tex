...
A \textit{CommonRoad} scenario contains one or more elements of type \textit{lanelet}, \textit{staticObstacle}, \textit{dynamicObstacle}, \textit{trafficSign}, \textit{trafficLight}, \textit{intersection}, and \textit{planningProblem}. 
Each has a unique\footnote{Unique within the whole scenario.} ID (of type positive integer) making it possible to reference it. 
Additionally, we can model meta information about map location and environment (e.g., weather conditions).
...
%		.3 [1..N] goalState.
%	}
%	\caption{Structure encoding general scenario information.}
...
\subsection{CommonRoad Map}

\subsection{Map Information}
The \textit{CommonRoad} map information has the following attributes:
\begin{itemize}
...
\end{itemize}
\subsection{CommonRoad Dynamic}

\subsection{CommonRoad Scenario}
...
We use the following auxiliary geometry elements:
...
\begin{figure}[!htpb]
	...
	\caption{Auxiliary elements used for defining scenarios.}
	\label{fig:auxiliary}
\end{figure}
...
In order to represent the graph of the road network, the elements \textit{predecessor}, \textit{successor}, \textit{adjacentLeft}, and \textit{adjacentRight} are used, which are omitted if they are empty (see Fig.~\ref{fig:structure}). Since these elements only contain objects which are already existing, we refrain from copying their data but introduce references to the neighboring lanelets by an attribute referring to their unique ID.
...
%As proposed in \cite{Bender2014}\footnote{\href{https://github.com/phbender/liblanelet}{github.com/phbender/liblanelet}}, format is inspired by the data structure of OpenSteetMap\footnote{\href{www.openstreetmap.org}{openstreetmap.org}} (OSM), which uses the elements \textit{nodes}, \textit{ways} and \textit{relations}.
