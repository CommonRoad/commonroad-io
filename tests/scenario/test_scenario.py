...
class TestScenario(unittest.TestCase):
    def setUp(self):
        ...
        self.lanelet_network = LaneletNetwork().create_from_lanelet_list(
            list([self.lanelet1, self.lanelet2])
        )
        ...
        self.dyn_set_obs = DynamicObstacle(
            1,
            ObstacleType("unknown"),
            initial_state=self.traj_pred.trajectory.state_at_time_step(0).convert_state_to_state(
                InitialState()
            ),
    ...
    def test_add_objects(self):
        ...
        self.assertEqual(
            expected_id_intersection, self.scenario.lanelet_network.intersections[0].intersection_id
        )
        ...
        self.assertEqual(
            expected_id_traffic_light,
            self.scenario.lanelet_network.traffic_lights[0].traffic_light_id,
        )

        self.scenario.add_objects(self.traffic_sign)
        self.assertEqual(
            expected_id_traffic_sign, self.scenario.lanelet_network.traffic_signs[0].traffic_sign_id
        )
        ...
        self.assertEqual(
            expected_id_intersection_incoming3,
    ...
    def test_remove_lanelet_network(self):
        ...
        self.scenario.add_objects(
            [self.traffic_sign1, self.traffic_sign2, self.traffic_sign3, self.traffic_sign4]
        )
        self.scenario.add_objects(
            [
                self.traffic_light100,
                self.traffic_light101,
                self.traffic_light102,
                self.traffic_light103,
            ]
        )
    ...
    def test_remove_lanelet(self):
        ...
        self.scenario.add_objects(
            self.lanelet2
        )  # add again to check whether ID was removed successfully
        self.assertEqual(len(self.scenario.lanelet_network.lanelets), 1)
    ...
    def test_remove_intersection(self):
        ...
        self.assertFalse(
            self.scenario._is_object_id_used(self.intersection.incomings[0].incoming_id)
        )
        self.assertFalse(
            self.scenario._is_object_id_used(self.intersection.incomings[1].incoming_id)
        )
        self.scenario.add_objects(
    ...
    def test_obstacle_by_id(self):
        ...
        self.assertEqual(
            static_obs1.obstacle_role, self.scenario.obstacle_by_id(-100).obstacle_role
        )
        self.assertEqual(
            static_obs1.obstacle_type, self.scenario.obstacle_by_id(-100).obstacle_type
        )
        np.testing.assert_array_almost_equal(
            static_obs1.initial_state.position,
        ...
        np.testing.assert_array_almost_equal(
            static_obs2.initial_state.position,
        ...
        self.assertEqual(
            static_obs3.obstacle_role, self.scenario.obstacle_by_id(5000).obstacle_role
        )
        self.assertEqual(
            static_obs3.obstacle_type, self.scenario.obstacle_by_id(5000).obstacle_type
        )
        np.testing.assert_array_almost_equal(
            static_obs3.initial_state.position,
        ...
        np.testing.assert_array_almost_equal(
            dyn_set_obs1.initial_state.position,
        ...
        self.assertEqual(
            dyn_set_obs2.obstacle_role, self.scenario.obstacle_by_id(-20).obstacle_role
        )
        self.assertEqual(
            dyn_set_obs2.obstacle_type, self.scenario.obstacle_by_id(-20).obstacle_type
        )
        np.testing.assert_array_almost_equal(
            dyn_set_obs2.initial_state.position,
            self.scenario.obstacle_by_id(-20).initial_state.position,
        )
    ...
    def test_obstacles_by_role_and_type(self):
        ...
        self.scenario.add_objects(
            [static_obs1, static_obs2, static_obs3, dyn_set_obs1, dyn_set_obs2]
        )
        # self.scenario.add_objects(self.lanelet_network)
        ...
        output_four = self.scenario.obstacles_by_role_and_type(
            ObstacleRole.DYNAMIC, ObstacleType("car")
        )
    ...
    def test_obstacles_by_position_intervals(self):
        ...
        self.scenario.add_objects(
            [static_obs1, static_obs2, static_obs3, static_obs4, dyn_set_obs1]
        )

        obstacles_in_interval = self.scenario.obstacles_by_position_intervals(
    ...
    def test_translate_rotate(self):
        ...
        np.testing.assert_array_almost_equal(
            expected_initial_state_position, self.scenario.obstacles[0].initial_state.position
        )
    ...
    def test_obstacle_states_at_time_step(self):
        exp_states_time_zero = {
            1: self.dyn_set_obs.state_at_time(0),
            2: self.dyn_traj_obs.state_at_time(0),
            0: self.static_obs.initial_state,
        }
        exp_states_time_one = {
            2: self.dyn_traj_obs.state_at_time(1),
            0: self.static_obs.initial_state,
        }
        self.scenario.add_objects(self.static_obs)
        self.scenario.add_objects(self.dyn_traj_obs)
        ...
        self.assertEqual(
            len(exp_states_time_one), len(self.scenario.obstacle_states_at_time_step(1))
        )
        self.assertEqual(
            exp_states_time_one[2].velocity,
            self.scenario.obstacle_states_at_time_step(1)[2].velocity,
        )
        self.assertEqual(
            exp_states_time_one[2].position[0],
            self.scenario.obstacle_states_at_time_step(1)[2].position[0],
        )
        self.assertEqual(
            exp_states_time_one[2].position[1],
            self.scenario.obstacle_states_at_time_step(1)[2].position[1],
        )
        self.assertEqual(
            exp_states_time_one[0].velocity,
            self.scenario.obstacle_states_at_time_step(1)[0].velocity,
        )
        self.assertEqual(
            exp_states_time_one[0].position[0],
            self.scenario.obstacle_states_at_time_step(1)[0].position[0],
        )
        self.assertEqual(
            exp_states_time_one[0].position[1],
            self.scenario.obstacle_states_at_time_step(1)[0].position[1],
        )

        # test for time step zero
        self.assertEqual(
            len(exp_states_time_zero), len(self.scenario.obstacle_states_at_time_step(0))
        )
        self.assertEqual(
            exp_states_time_zero[1].velocity,
            self.scenario.obstacle_states_at_time_step(0)[1].velocity,
        )
        self.assertEqual(
            exp_states_time_zero[1].position[0],
            self.scenario.obstacle_states_at_time_step(0)[1].position[0],
        )
        self.assertEqual(
            exp_states_time_zero[1].position[1],
            self.scenario.obstacle_states_at_time_step(0)[1].position[1],
        )
        self.assertEqual(
            exp_states_time_zero[2].velocity,
            self.scenario.obstacle_states_at_time_step(0)[2].velocity,
        )
        self.assertEqual(
            exp_states_time_zero[2].position[0],
            self.scenario.obstacle_states_at_time_step(0)[2].position[0],
        )
        self.assertEqual(
            exp_states_time_zero[2].position[1],
    ...
    def test_assign_vehicles(self):
        states = list()
        states.append(KSState(time_step=0, orientation=0, position=np.array([1, 0.5]), velocity=5))
        states.append(KSState(time_step=1, orientation=0, position=np.array([1, 0.5]), velocity=10))
        trajectory = Trajectory(0, states)
        ...
        dyn_traj_obs = DynamicObstacle(
            2,
            ObstacleType("unknown"),
            initial_state=traj_pred.trajectory.state_at_time_step(0).convert_state_to_state(
                InitialState()
            ),
        ...
        self.assertEqual(
            exp_dynamic_lanelet_of_obstacle[0],
            scenario_tmp.obstacle_by_id(2).initial_center_lanelet_ids,
        )
        self.assertEqual(
            exp_dynamic_lanelet_of_obstacle,
        ...
        scenario_tmp: Scenario = deepcopy(sc)
        dyn_traj_obs_3 = DynamicObstacle(
            3,
            ObstacleType("unknown"),
            initial_state=traj_pred.trajectory.state_at_time_step(0).convert_state_to_state(
                InitialState()
...
class TestScenarioID(unittest.TestCase):
    ...
    def test_to_string(self):
        ids = [
            ScenarioID(),
            ScenarioID(obstacle_behavior="T"),
            ScenarioID(map_name="Illegal-Character_"),
        ]
        for scenario_id in ids:
            benchmark_id = str(scenario_id)
            scenario_id_parsed = ScenarioID.from_benchmark_id(benchmark_id, "2020a")
            self.assertEqual(
                scenario_id, scenario_id_parsed, f"{scenario_id} != {scenario_id_parsed}"
            )
...
class TestEnvironment(unittest.TestCase):
    def test_equality(self):
        environment_1 = Environment(
            Time(8, 30), TimeOfDay.MORNING, Weather.CLEAR, Underground.CLEAN
        )
        ...
        environment_2 = Environment(
            Time(9, 30), TimeOfDay.MORNING, Weather.CLEAR, Underground.CLEAN
        )
        self.assertFalse(environment_1 == environment_2)

        environment_2 = Environment(
            Time(8, 30), TimeOfDay.UNKNOWN, Weather.CLEAR, Underground.CLEAN
        )
        self.assertFalse(environment_1 == environment_2)
        ...
        environment_2 = Environment(
            Time(8, 30), TimeOfDay.AFTERNOON, Weather.CLEAR, Underground.DIRTY
        )
        self.assertFalse(environment_1 == environment_2)

        environment_2 = Environment(
            Time(8, 30), TimeOfDay.MORNING, Weather.MID_RAIN, Underground.SNOW
        )
        self.assertFalse(environment_1 == environment_2)
        ...
        environment_2 = Environment(
            Time(8, 30), TimeOfDay.MORNING, Weather.CLOUDY, Underground.CLEAN
        )
        self.assertFalse(environment_1 == environment_2)
    ...
    def test_hash(self):
        ...
        environment_2 = Environment(
            Time(8, 30), TimeOfDay.SUNSET, Weather.HEAVY_RAIN, Underground.DAMAGED
        )
        self.assertNotEqual(hash(environment_1), hash(environment_2))

        environment_2 = Environment(
            Time(8, 30), TimeOfDay.MORNING, Weather.LIGHT_RAIN, Underground.CLEAN
        )
        self.assertNotEqual(hash(environment_1), hash(environment_2))
