...
class TestObstacle(unittest.TestCase):
    ...
    def test_initialize_dynamic_obstacle(self):
        ...
        dynamic_obstacle = DynamicObstacle(obstacle_id=obstacle_id, obstacle_type=obstacle_type, prediction=prediction,
                                           ...
                                           external_dataset_id=external_dataset_id, history=history,
    ...
    def test_update_initial_state(self):
        ...
        dynamic_obs.update_initial_state(current_state, current_signal_state, current_center_lanelet_ids,
        ...
        self.assertEqual(dynamic_obs.initial_signal_state, current_signal_state,
                         "Initial signal state should be updated")
        self.assertEqual(dynamic_obs.initial_center_lanelet_ids, current_center_lanelet_ids,
                         "Initial center lanelet ids should be updated")
        self.assertEqual(dynamic_obs.initial_shape_lanelet_ids, current_shape_lanelet_ids,
        ...
        self.assertEqual(dynamic_obs.signal_history, [initial_signal_state],
                         "Old initial signal state should be appended to history")
        self.assertEqual(dynamic_obs.center_lanelet_ids_history[-1], initial_center_lanelet_ids,
                         "Old initial center lanelet ids should be appended to history")
        self.assertEqual(dynamic_obs.shape_lanelet_ids_history[-1], initial_shape_lanelet_ids,
    ...
    def test_update_initial_state_delete(self):
        ...
        max_length = 2
        dynamic_obs.update_initial_state(current_state, current_signal_state, current_center_lanelet_ids,
                                         current_shape_lanelet_ids, max_history_length=max_length)
        self.assertLessEqual(len(dynamic_obs.history), max_length, "History should be truncated")
        self.assertEqual(dynamic_obs.history, history[1:] + [initial_state],
                         "History should be shifted to include current state")
        self.assertEqual(dynamic_obs.signal_history, signal_history[1:] + [initial_signal_state],
                         "Signal history should be shifted to include current signal state")
        self.assertEqual(dynamic_obs.center_lanelet_ids_history,
        ...
        self.assertEqual(dynamic_obs.shape_lanelet_ids_history,
    ...
    def test_update_initial_state_assertion(self):
        ...
        dynamic_obs = DynamicObstacle(obstacle_id=30, obstacle_type=ObstacleType.CAR, obstacle_shape=rect,
                                      initial_state=initial_state)
    ...
    def test_update_prediction(self):
        ...
        initial_signal_state = SignalState(braking_lights=True, time_step=0)
        dynamic_obs = DynamicObstacle(obstacle_id=30, obstacle_type=ObstacleType.CAR, obstacle_shape=rect,
                                      initial_state=initial_state, initial_signal_state=initial_signal_state)
