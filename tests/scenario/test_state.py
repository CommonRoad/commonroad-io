...
class TestState(unittest.TestCase):
    ...
    def test_used_attributes(self):
        ...
        state = PMState(position=np.array([0.0, 0.0]), velocity_y=10.0)
    ...
    def test_is_uncertain_orientation(self):
        state = STState(orientation=AngleInterval(0.0, 0.9))
    ...
    def test_fill_with_defaults(self):
        ...
        for field in initial_state.attributes:
            ...
            if field == "position":
                self.assertEqual(np.array([0.0, 0.0]).tolist(), value.tolist())
            else:
    ...
    def test_equals(self):
        ks_state_1 = STState(
            ...
            steering_angle=0.1,
            velocity=10.0,
            orientation=AngleInterval(0.0, 0.2),
        )
        ks_state_2 = STState(
            time_step=0,
            position=np.array([0.0, 0.0]),
        ...
        pm_state_2.velocity = Interval(5.0, 20.0)
