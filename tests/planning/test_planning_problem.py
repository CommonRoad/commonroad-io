...
class TestTranslateRotate(unittest.TestCase):
    ...
    def test_rotate(self):
        ...
        self.assertAlmostEqual(
            planning_problem.goal.state_list[0].orientation.start, angle + np.pi / 8
        )
        self.assertAlmostEqual(
            planning_problem.goal.state_list[0].orientation.end, angle + 3 * np.pi / 8
        )
        self.assertAlmostEqual(
            planning_problem.goal.state_list[1].orientation.start, angle + 3 * np.pi / 4
        )
    ...
    def test_goal_reached(self):
        ...
        goal_state_2 = STState(
            position=shape2, time_step=Interval(0, 2), orientation=AngleInterval(0.0, 0.1)
        )
        goal_region = GoalRegion([goal_state_1, goal_state_2])
        planning_problem = PlanningProblem(1, initial_state, goal_region)
