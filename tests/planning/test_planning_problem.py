...
from commonroad.scenario.state import STState, KSState, InitialState
...
class TestTranslateRotate(unittest.TestCase):
    ...
    def test_goal_reached(self):
        ...
        goal_state_1 = STState(position=shape1, time_step=Interval(0, 5), orientation=AngleInterval(0., 0.1))
        goal_state_2 = STState(position=shape2, time_step=Interval(0, 2), orientation=AngleInterval(0., 0.1))
        goal_region = GoalRegion([goal_state_1, goal_state_2])
        planning_problem = PlanningProblem(1, initial_state, goal_region)
