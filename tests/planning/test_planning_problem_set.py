...
class TestPlanningProblemSet(unittest.TestCase):
    ...
    def test_add_planning_problem(self):
        state_1 = InitialState(
            time_step=1, position=np.array([3, 5]), velocity=14, orientation=0, yaw_rate=2, slip_angle=4
        )
        state_2 = InitialState(
            time_step=2, position=np.array([3, 5]), velocity=14, orientation=0, yaw_rate=2, slip_angle=4
        )
