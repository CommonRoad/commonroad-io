...
for path in list(Path(__file__).parent.glob("*.xml")):
    ...
    if file_name_new.exists():
        ...
        file_writer = CommonRoadFileWriter(scenario, planning_problem_set, file_format=FileFormat.PROTOBUF)
        file_writer.write_to_file(str(file_name_new), overwrite_existing_file=OverwriteExistingFile.ALWAYS)
