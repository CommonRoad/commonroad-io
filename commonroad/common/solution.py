...
class TrajectoryType(Enum):
    ...
    def get_trajectory_type(
        cls, trajectory: Trajectory, desired_vehicle_model: VehicleModel = None
    ) -> "TrajectoryType":
        """
...
class Solution:
    ...
    def computation_time(self, computation_time):
        if computation_time is not None:
            ...
            assert is_positive(computation_time), "<Solution> computation_time needs to be positive!".format(
...
class CommonRoadSolutionReader:
    ...
    def _parse_solution(cls, root_node: et.Element) -> Solution:
        ...
        pp_solutions = [
            cls._parse_planning_problem_solution(vehicle_ids[idx], cost_ids[idx], trajectory_node)
    ...
    def _parse_state(cls, state_type: StateType, state_node: et.Element) -> TraceState:
        ...
        state_types = {
            StateType.MB: MBState,
            StateType.KS: KSState,
            StateType.PM: PMState,
            StateType.ST: STState,
            StateType.Input: InputState,
            StateType.PMInput: PMInputState,
        }
    ...
    def _serialize_solution(cls, solution: Solution) -> et.Element:
        ...
        for pp_solution in solution.planning_problem_solutions:
            trajectory_node = cls._create_trajectory_node(
                pp_solution.trajectory_type, pp_solution.planning_problem_id, pp_solution.trajectory
            )
