...
class visual():
    ...
    def zoom_out(self):
        self.xmin += 10*sign(self.xmin)
        self.xmax += 10*sign(self.xmax)
        self.ymin += 10*sign(self.ymin)
        self.ymax += 10*sign(self.ymax)
    ...
    def construction(self, scenario: Scenario, i: int) -> list:
        ...
        for t in range(self.scenario.dynamic_obstacles[i].prediction.final_time_step):
            ...
            list = [  # top
                (- length * 0.5, - width * 0.5, tope), (- length * 0.5, + width * 0.5, tope),
                (+ length * 0.0, + width * 0.5, tope), (+ length * 0.0, - width * 0.5, tope)]
            ...
            list = [  # right
                (- length * 0.5, + width * 0.5, top), (- length * 0.5, + width * 0.5, tope),
                (+ length * 0.0, + width * 0.5, tope), (+ length * 0.0, + width * 0.5, top)]
            ...
            list_patchcollection = []
            for list in biglist:
                patchcollection = Poly3DCollection(list, linewidth=0, edgecolor="k", facecolor=colors, rasterized=True,zorder=10)
                self.ax.add_collection3d(patchcollection)
