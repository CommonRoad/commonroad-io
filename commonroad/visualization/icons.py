...
from commonroad.geometry.shape import Shape, SemiTrailerTruck
...
def _create_trailer_vertices(shape: SemiTrailerTruck) -> List[np.ndarray]:
    trailer_dist_from_front_to_origin = (shape.trailer_dist_from_front_to_hitch
    ...
    v_trailer = np.array([[x_min, -y_min_max], [x_max, -y_min_max], [x_max, y_min_max], [x_min, y_min_max]])
    return [v_trailer]
...
def rotate_by_hitch_angle(vertices: np.ndarray, shape: SemiTrailerTruck, hitch_angle: float) -> np.ndarray:
    hitch_point = np.array([shape.truck_dist_from_rear_to_hitch - shape.truck_dist_from_rear_to_rear_axle, 0])
    translated_to_hitch_position = rotate_translate(vertices, -hitch_point, 0.)
    rotated_by_hitch_angle = rotate_translate(translated_to_hitch_position, np.zeros(2), hitch_angle)
    translated_back = rotate_translate(rotated_by_hitch_angle, hitch_point, 0.)
...
def draw_truck_icon(
    ...
    patch_list += [
        create_origin_patch(state.position)
    ]
