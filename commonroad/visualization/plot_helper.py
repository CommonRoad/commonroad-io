...
# 1. set backend for matplotlib (Qt5Agg or TkAgg recommended) directly after 
...
def example_plot():        
    ...
    for i in range(0, nrun):
        ...
        plot_limits=None, draw_params={'time_begin':i})
...
def redraw_obstacles(scenario: Scenario,
                     handles: Dict[int, List[mpl.patches.Patch]],
                     figure_handle: mpl.figure.Figure, draw_params=None,
                     plot_limits: Union[List[Union[int, float]], None] = None,
                     draw: bool = True) -> None:
    """
    This function is used for fast updating dynamic obstacles of an already
    drawn plot. Saves on average about 80% time
    compared to a complete plot.
    Deletes all dynamic obstacles which are specified in handles and draws
    dynamic obstacles of a scenario.

    :param scenario: scenario from which obstacle should be plotted
    :param handles: dict of obstacle_ids and corresponding patch handles (
    generated by draw_object function)
    ...
    if plot_limits is not None:
        draw_object(scenario.obstacles_by_position_intervals(
                ...
                draw_params=draw_params, plot_limits=plot_limits,
                handles=handles)
    else:
        draw_object(scenario.obstacles, draw_params=draw_params,
                    plot_limits=plot_limits, handles=handles)
    ...
    if draw is True:
        ...
        if mpl.get_backend() == 'TkAgg':
            ...
            try:
                ...
                raise Exception(
                        '<plot_helper/redraw_dynamic_obstacles> Backend for '
                        'matplotlib needs to be \'Qt5Agg\' or \'TkAgg\' but is'
                        '\'%s\'' % mpl.get_backend())
