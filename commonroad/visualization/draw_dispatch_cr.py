...
def _create_drawers_dict() -> Dict[type, Callable]:
    """
    Creates default dict of drawer functions, which are later called for draw
    objects depending on their class
...
def _retrieve_value_by_path(style_sheet_caller: dict,
                            value_path: Tuple[str, ...]):
    """
    Retrieves value corresponding to value path from the nested dict
    style_sheet.
...
def _retrieve_value(style_sheet: dict, call_stack: tuple,
                    value_path: Tuple[str, ...]):
    """
    Retrieves value corresponding to value_path from the nested dict
    style_sheet. If value_path not contained in
    style_sheet, try to retrieve from default draw_params.
    Starts by searching for value_path beginning at first element in call
    stack, stepping down all frames
    ...
    for idx_r in range(0, len(call_stack)):
        ...
        try:
            ...
            continue
    # try to retrieve value without call_stack (for fallback to top-level
    # parameters)
    try:
    ...
    # if value not found yet, it is neither given in default parameters nor
    # in provided parameters,
...
def _retrieve_alternate_value(style_sheet: dict, call_stack: Tuple[str, ...],
                              value_path_1: Tuple[str, ...],
    ...
    for idx_r in range(0, len(call_stack)):
        ...
        try:
            for idx in range(0, len(call_stack) - idx_r):
                style_sheet_caller = style_sheet_caller[call_stack[idx]]
            try:
                value = _retrieve_value_by_path(style_sheet_caller,
                                                value_path_1)
                return value
            except KeyError:
                pass
    ...
    # try to retrieve from default parameters
    try:
        value = _retrieve_value_by_path(default_draw_params,
                                        call_stack + value_path_1)
        return value
    except KeyError:
        pass
...
plottable_types = Union[
    list, Scenario, Trajectory, LaneletNetwork, Lanelet, Obstacle,
    ShapeGroup, Shape, GoalRegion, PlanningProblem, PlanningProblemSet,
    State, Occupancy, TrafficSign]
...
def draw_object(obj: Union[plottable_types, List[plottable_types]],
                ...
                legend: Union[Dict[Tuple[str, ...], str], None] = None) -> \
        Union[None, List[mpl.patches.Patch]]:
    """
    ...
    assert isinstance(plot_limits, (list,
                                    np.ndarray)) or plot_limits is None, \
        '<visualization/draw_dispatch_cr/draw_object>:' \
    ...
    if plot_limits is not None:
        assert np.less(plot_limits[0], plot_limits[1]) and np.less(
                plot_limits[2], plot_limits[
                    3]), '<draw_dispatch_cr/draw_object>: position limits ' \
                         'need to be given by [x_min, x_max, y_min, y_max]'
    ...
    if type(obj) is list:
        if len(obj) == 0:
            return []
        elif isinstance(obj[0], (
                DynamicObstacle, Trajectory, Rectangle, Polygon, Lanelet,
                Circle)):
            patch_list = draw_func[type(obj[0])](obj, plot_limits, ax,
                                                 draw_params, draw_func,
                                                 handles, call_stack)
            return patch_list
        else:
            patch_list = list()
            for o in obj:
                patch_list_tmp = draw_object(o, None, ax, draw_params,
    ...
    if type(obj) in draw_func.keys():
        # plotting the object
        patch_list = draw_func[type(obj)](obj, plot_limits, ax, draw_params,
                                          draw_func, handles, call_stack)
        return patch_list
    else:
        for obj_type, func in draw_func.items():
            if isinstance(obj, obj_type):
                patch_list = func(obj, plot_limits, ax, draw_params, draw_func,
                                  handles, call_stack)
                return patch_list
