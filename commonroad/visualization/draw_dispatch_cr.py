...
def _add_legend(legend: Dict[Tuple[str, ...], str], draw_params):
    pass  # """  # Adds legend with color of objects specified by
    # legend.keys() and texts specified by legend.values().  # :param legend:
    # color of objects specified by path in legend.keys() and texts specified
    # by legend.values()  # :param draw_params: draw parameters used for
    # plotting (color is extracted using path in legend.keys())  # :return:
    # """  # handles = []  # for obj_name, text in legend.items():  #
    # try:  #         color = _retrieve_value(draw_params, (), obj_name)  #
    # except:  #         color = None  #     if color is not None:  #
    # handles.append(mpatches.Patch(color=color, label=text))  #  # l =
    # plt.legend(handles=handles)  # l.set_zorder(1000)
